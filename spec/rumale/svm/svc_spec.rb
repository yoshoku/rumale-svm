...
RSpec.describe Rumale::SVM::SVC do
  let(:x) { dataset[0] }
  let(:y) { dataset[1] }
  ...
  context 'when called predict method before training with fit method' do
    ...
    it 'raises Runtime error', :aggregate_failures do
      expect { svc.predict(x) }.to raise_error(
        RuntimeError, 'Rumale::SVM::SVC#predict expects to be called after training the model with the fit method.'
      )
      expect { svc.predict_proba(x) }.to raise_error(
        RuntimeError, 'Rumale::SVM::SVC#predict_proba expects to be called after training the model with the fit method.'
      )
      expect { svc.decision_function(x) }.to raise_error(
        RuntimeError, 'Rumale::SVM::SVC#decision_function expects to be called after training the model with the fit method.'
      )
    end
  end
end
